...
class IntegralModel extends Model{
    public function getAll($conditons){
        ...
        //获取总条数
        $c_sql = "select count(*) from integral " . $where;
        ...
        $sql = "SELECT * FROM integral " . $where .
...
//
//        //获取总条数
//        $count = $this->db->fetchColumn("select count(*) from integral WHERE user_id={$_SESSION['user_info1']['user_id']}");
//
//        //每页显示多少条
//        $pageSize = 2;
//
//        $start = ($page-1)*$pageSize;
//        $sql = "select * from integral WHERE user_id={$_SESSION['user_info1']['user_id']} order by `time` desc limit $start,$pageSize";
//        //2.执行sql语句
//        $rows = $this->db->fetchAll($sql);
