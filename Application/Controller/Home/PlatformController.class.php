...
class PlatformController extends Controller
{
    public function __construct()
    {
        ...
        if($result === false){
            //跳转到登陆页面
            $this->alert( "没有登陆", "index.php?p=Home&c=Login&a=login" );
//            $this->redirect("index.php?p=Home&c=Login&a=login","没有登陆",3);
        }
    }
    ...
    private function checkLogin(){
        ...
        if(!isset($_SESSION['user_info1'])){
            ...
            if(isset($_COOKIE['id']) && isset($_COOKIE['password'])){
                ...
                $result = $adminModel->checkByCookie2($id,$password);
//                var_dump($result);exit;
                if($result !== false){//验证成功
                    //记录登陆信息到session中
                    $_SESSION['user_info1'] = $result;
                    return true;
                }
            }
            //3. 如果没有,跳转到登陆页面
